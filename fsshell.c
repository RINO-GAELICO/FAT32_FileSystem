...
int cmd_mv (int argcnt, char *argvec[])
	...
	char * currentPath;
    char* pathnameVar;
    pathnameVar = malloc(strlen(src)+2);
    currentPath = malloc(MAX_DEPTH_PATH);

	if(src[0]!='/'){ // relative path
        ...
        strcpy(pathnameVar,src);
        DEholder = parsePath(pathnameVar, DEholder);
    }
	

	if(DEholder!=NULL && DEholder->indexDE>=0 && DEholder->isDir==0){ // valid path exist and it is a file
	...
	}else { // not a valid path to a file
		printf("Not a Valid Path to a File");
		free(pathnameVar);
		free(currentPath);
		if(DEholder!=NULL){
			free(DEholder->currentComponent);
